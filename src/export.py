    ...
    def __init__(self, baseline_model):
        ...
        for name, layer in baseline_model.feature_block.named_children():
            if isinstance(layer, nn.Linear):
                ...
                quantized_layer.weight.data = int8_weights.to(torch.float32) * scale  # Fixed de-quantization hook
                ...
                self.feature_block.append(torch.serialization.skip_module_hook if hasattr(torch.serialization, 'skip_module_hook') else layer)
            else:
...
def compile_production_quantized_package():
    ...
    if not os.path.exists(weight_path):
        print(f"   [Error] Baseline model weights not found at '{weight_path}'. cannot proceed.")
        return
    ...
    print("   -> Freezing continuous scales and compiling true INT8 parameter weights...")
    ...
    print(f"   -> Realized Storage Compression: {compression_ratio:.2f}x smaller footprint")
    print(f"   ------------------------------------------------------------------")
