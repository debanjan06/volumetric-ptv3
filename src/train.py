...
def run_production_training():
    ...
    dataset = DALESProductionDataset(
        data_directory=train_dir, max_points_per_block=8192, chunks_per_file=32
    )
    ...
    for epoch in range(1):
        ...
        for batch_idx, (coords, features, labels) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 10 == 0 or (batch_idx + 1) == 464:
                print(
                    f"   [Batch {batch_idx + 1} / 464] Current Loss Convergence: {last_computed_loss:.5f}"
                )
    ...
    if last_computed_loss is not None:
        ...
        if os.path.exists(test_dir) and os.listdir(test_dir):
            model = model.cpu()
            _ = evaluator.execute_validation_pass(
