...
def render_complete_urban_scene():
    ...
    dataset = DALESProductionDataset(data_directory=test_dir, max_points_per_block=8192, chunks_per_file=32)
    ...
    print("   -> Stitching aerial chunks and streaming full-scale inference across the neighborhood...")
    ...
    print(f"   -> Assembling final scene graphics layer ({len(canvas_xyz)} total 3D points)...")
    
    # 2. Render the complete stitched environment
    fig = pgo.Figure()
    fig.add_trace(pgo.Scatter3d(
        x=canvas_xyz[:, 0], y=canvas_xyz[:, 1], z=canvas_xyz[:, 2],
        mode='markers',
        marker=dict(
