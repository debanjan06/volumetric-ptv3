...
def export_to_production_onnx():
    ...
    if not os.path.exists(weight_path):
        print(
            f"   [Error] Checkpoint '{weight_path}' not found. Please verify the file path."
        )
        return
    ...
    torch.onnx.export(
        ...
        dynamic_axes={
            "input_point_stream": {
                1: "point_cloud_density"
            },  # Enable dynamic processing capacities for varying block densities
    ...
    print("   -> Graph Compilation State    : SUCCESS")
    print(
        "   -> Deployment Target Readiness: Optimized for ONNX Runtime / TensorRT NPU"
    )
    print("====================================================================")
