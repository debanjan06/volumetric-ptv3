...
class DALESCrossValidator:
    ...
    def execute_validation_pass(self, test_directory, weight_path):
        ...
        dataset = DALESProductionDataset(
            data_directory=test_directory, max_points_per_block=8192, chunks_per_file=32
        )
        ...
        model = BimodalQATLinear(
            in_features=14, out_features=self.num_classes, bit_width=8
        )
...
if __name__ == "__main__":
    ...
    if os.path.exists(weight_path):
        ...
        validator.execute_validation_pass(
            test_directory=test_dir, weight_path=weight_path
        )
    else:
        print(
            f"   [Error] Saved weight checkpoint not found at '{weight_path}'. Run training first."
        )
